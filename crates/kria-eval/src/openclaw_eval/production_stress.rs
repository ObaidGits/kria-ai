...
pub fn registry_workload(installs: usize, updates: usize, removals: usize) -> Result<(usize, usize, usize), String> {
    ...
    let audit = Arc::new(
        kria_core::openclaw::audit::AuditLedger::open(&db_path, b"stress-key".to_vec()).map_err(|e| e.to_string())?,
    );
    ...
    for i in 0..installs {
        ...
        let root = author_signed_bundle(&author_dir, &slug, seed).map_err(|e| format!("author {slug}: {e}"))?;
...
mod tests {
    ...
    fn stress_registry_50_installs_50_updates_50_removals() {
        let (installs, updates, removals) = registry_workload(50, 50, 50).expect("registry workload");
    ...
    async fn stress_100_prompts_plus_parallel_real_docker() {
        ...
        assert_eq!(seq_ok, 100, "all 100 sequential prompt executions must succeed");
        ...
        for i in 0..20 {
            ...
            handles.push(tokio::spawn(async move {
                for attempt in 0..40u32 {
                    let mut graph = ExecutionGraph::new(format!("g-par-{i}-{attempt}"), format!("goal-par-{i}"));
                    graph.add_node(GraphNode::new(
                        "calc",
                        NodeKind::Skill {
                            provider_id: "openclaw".to_string(),
                    ...
                    let ctx = ExecutionContext::new(format!("goal-par-{i}"), format!("corr-par-{i}-{attempt}"));
                    if engine.execute_graph(&graph, &ctx).await.status == ScheduleStatus::Completed {
                        return true;
                    }
        ...
        assert_eq!(after, baseline, "container count must return to baseline (0 leak)");
    }
}
