...
mod tests {
    ...
    async fn r4_execute_matched_skill_real_docker_e2e() {
        ...
        assert_eq!(
            ...
            "matched skill must execute successfully: {result:?}"
        );
    ...
    // NOTE: the legacy `r4_no_match_declines_cleanly_real` test drove the deleted
    // `SemanticSkillRouter`. Honest no-match decline is now a CPP concern —
    // covered by `kria-core tools::capability_dispatch::tests::dispatcher_no_match_is_honest`.
    ...
    async fn bundle_execution_mounted_skill_runs_in_real_container() {
        ...
        let audit =
            Arc::new(AuditLedger::open(&db_path, b"bundle-exec-key".to_vec()).expect("audit"));
        ...
        assert!(
            bridge_dir.join("oc_bundle_exec_fixture.json").exists()
                && bridge_dir.join("handler.js").exists(),
