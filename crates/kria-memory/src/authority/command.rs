...
//! ([`crate::authority`]) consumes an already-validated envelope.
//!
...
use crate::model::{CallerContext, GraphRevision, IdempotencyKey, InvocationId};
use crate::types::MemoryMode;

pub use crate::authority::CommandKind;

/// Build a canonical-encoding validation error (`StorageError::Encoding`).
fn encoding_err(msg: impl Into<String>) -> crate::error::MemoryError {
...
impl Deadline {
    ...
    pub fn deadline_from(
        &self,
        start: crate::model::UtcTimestamp,
    ) -> crate::model::UtcTimestamp {
        let end = start.as_datetime() + chrono::Duration::milliseconds(self.budget_ms as i64);
        crate::model::UtcTimestamp::from_datetime(end)
    }
}
...
mod tests {
    ...
    fn deadline_is_bounded() {
        ...
        let start =
            crate::model::UtcTimestamp::from_rfc3339_utc("2026-01-01T00:00:00Z").unwrap();
