...
//!   ([`crate::runtime_backend::KriaMemoryRuntime`] fact writes, the
//!   legacy [`crate::write_policy`] `Memory` path, the direct
//!   [`crate::goals::GoalStore`] table writes).
//! * **F2.2 — relation registry + Memory Links canonical.** Replaces the
//!   free-text [`crate::types::Relationship`] edge and the ad-hoc graph
//!   entity read model with the registry-governed relationship + typed
//!   endpoints over [`crate::model::Entity`].
//! * **F3 — retrieval-on-v2.** Repoints retrieval/search/graph reads (and the
//!   API DTOs they feed) at the v2 [`crate::model::Record`] /
//!   [`crate::model::Entity`] / [`crate::model::Goal`]
...
//! |---|---|---|---|
//! | [`crate::types::Memory`] | [`crate::model::Record`] (`RecordKind::Memory`) + [`crate::model::Provenance`] | legacy write policy, [`crate::stores`] `sqlite_memory`, retrieval, `api` | F1.5 write + F3 read |
//! | [`crate::runtime_types::MemoryFact`] | [`crate::model::Record`] (`RecordKind::Memory`); decay/access become retrieval-time scoring, not stored columns | [`crate::runtime_backend::KriaMemoryRuntime`], [`crate::manager`] `MemoryManager`/`MemoryReader`, `tools::knowledge`, desktop history helpers | F1.5 write + F3 read |
//! | [`crate::types::Event`] | authority `events` + [`crate::model::EventId`] / [`crate::model::Provenance`] creation event | append-only event log, write path | F1.5 |
//! | [`crate::types::WriteCandidate`] / `WriteDecision` | `AuthorityTx` command inputs/outputs ([`crate::authority`]) | legacy write policy, tools | F1.5 |
//!
//! ## Provenance / source
//!
//! | Legacy | Canonical v2 | Live consumers | Gate |
//! |---|---|---|---|
//! | [`crate::types::Source`] (provenance-tag enum) | structured [`crate::model::Provenance`] ([`crate::model::Actor`] / [`crate::model::Method`] / [`crate::model::ModelIdentity`] / [`crate::model::Locator`] / [`crate::model::ParentRef`]) + [`crate::model::SourceRef`] | `types::Event`/`Memory`/`WriteCandidate`, write policy, extraction | F1.5 |
//! | *the consented-source concept* | [`crate::model::SourceRecord`] (`sources` row) with closed [`crate::authority::command::SourceKind`] | source trust/consent (F1) | F1.5 |
//!
//! ## Graph entities / relationships
//!
//! | Legacy | Canonical v2 | Live consumers | Gate |
//! |---|---|---|---|
//! | [`crate::types::Entity`] | [`crate::model::Entity`] (`entities_v2`) + [`crate::model::Alias`] / [`crate::model::Mention`] | [`crate::entity_resolution`], `api` graph methods, [`crate::stores`] `GraphStore`/`sqlite_graph` | F2.2 + F1.5 |
//! | [`crate::types::Relationship`] | F2.2 registry-governed relationship + [`crate::model::Evidence`] (no v2 struct yet — added in F2.2) | [`crate::extraction`], `api`, `GraphStore`/`sqlite_graph` | **Deleted F2.2.7** |
//! | [`crate::types::GraphHit`] | F2.2/F3 traversal result over v2 [`crate::model::Entity`] | `api` graph neighbourhood | **Deleted F2.2.7** |
//!
//! ## Goals
//!
//! | Legacy | Canonical v2 | Live consumers | Gate |
//! |---|---|---|---|
//! | [`crate::goals::Goal`] | [`crate::model::Goal`] (`goals_v2`) + [`crate::model::GoalProgress`] | `api` goal accessor, desktop goal commands, active-learning, dreaming | F1.5 + F3 |
//! | [`crate::goals::GoalStatus`] | [`crate::model::GoalStatus`] — **closed** set. Legacy `failed`/`abandoned` are **not** in the v2 set (which adds `conflicted`/`stale`/`superseded`/`deleted`); the cutover remaps `failed`/`abandoned` at F1.5 (see below). | as above | F1.5 |
//! | [`crate::goals::GoalStore`] / `NewGoal` / `GoalAnalytics` | governed goal commands over `goals_v2` (F1.5) | `api`, desktop commands | F1.5 |
//!
//! ## Legacy `GoalStatus` → v2 [`crate::model::GoalStatus`] remap (applied at F1.5)
//!
...
//!
//! * **`crate::authority::relationship_migration`** — the
...
//!
//! * **`crate::types::Relationship`** — legacy free-text edge struct
...
//! * **`crate::types::GraphHit`** — legacy BFS traversal result wrapping
