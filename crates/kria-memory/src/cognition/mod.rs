...
use crate::stores::ports::LlmClient;
use crate::types::{Availability, Source, WriteCandidate};
    ...
    pub async fn consolidate(
        ...
        for insight in insights {
            ...
            let cand = WriteCandidate {
                ...
                proposed_type: Some(crate::types::MemoryType::Reflection),
                derived_from: insight.derived_from.clone(),
                ..WriteCandidate::user(session_id, insight.content)
            };
            match self.write_policy.submit(cand) {
                Ok(crate::types::WriteDecision::Queued { .. })
                | Ok(crate::types::WriteDecision::Stored { .. }) => accepted += 1,
