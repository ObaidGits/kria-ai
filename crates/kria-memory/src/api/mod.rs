...
use crate::authority::integrity::RecoveryFault;
use crate::authority::{RevisionWake, WakePublisher};
use crate::cognition::Cognition;
use crate::db::Database;
use crate::error::{MemoryError, MemoryResult, RecoveryError, StorageError};
use crate::jobs::ConsolidationJob;
use crate::modes::ModeManager;
use crate::retriever::{RetrievalCtx, RetrievalResult, Retriever};
use crate::scheduler::{CognitiveScheduler, ResourceMonitor};
use crate::stores::ports::{Embedder, EventStore, LlmClient, RelationalStore, VectorStore};
use crate::stores::{
    SqliteEventStore, SqliteGraphStore, SqliteRelationalStore, SqliteSearchStore, SqliteVectorStore,
};
use crate::types::{Availability, MemoryMode, WriteCandidate, WriteDecision};
use crate::write_policy::admission::Admission;
use crate::write_policy::slow::SlowPath;
...
pub struct CognitiveReport {
    ...
    pub goals: crate::goals::GoalAnalytics,
    pub plans: crate::planning::PlanAnalytics,
...
impl MemorySystem {
    ...
    ///
    /// The narrow authority ports ([`crate::authority`]) exposed by the
    ...
    /// authenticated [`CallerContext`](crate::model::CallerContext) at
    ...
    /// The derived-projection [`OutboxPort`](crate::authority::OutboxPort)
    /// over the single injected authority handle (task F1.2.3).
    pub fn outbox(&self) -> crate::authority::SqliteOutbox {
        crate::authority::SqliteOutbox::new(self.db.clone())
    }

    /// The integrity/recovery inspection
    /// [`IntegrityPort`](crate::authority::IntegrityPort) over the single
    /// injected authority handle (task F1.2.3).
    pub fn integrity(&self) -> crate::authority::AuthorityIntegrity {
        crate::authority::AuthorityIntegrity::new(self.db.clone())
    }

    /// The governed [`AuthorityCommandBus`](crate::authority::AuthorityCommandBus)
    ...
    /// validated [`CommandEnvelope`](crate::authority::CommandEnvelope)
    /// through (build one with a
    /// [`CommandCandidate`](crate::authority::CommandCandidate) +
    /// [`WriteContext`](crate::authority::WriteContext)). Reusing the
    ...
    /// submit through
    /// [`AuthorityCommandBus::submit_deferred`](crate::authority::AuthorityCommandBus::submit_deferred).
    pub fn command_bus(
        &self,
    ) -> crate::authority::AuthorityCommandBus<RevisionWakeBroadcaster> {
        crate::authority::AuthorityCommandBus::with_publisher(
            self.db.clone(),
            self.wake_publisher(),
        )
    }
    ...
    fn assemble(
        ...
        let initial_state = {
            let checker = crate::authority::StartupIntegrityChecker::new(db.clone());
        ...
        let retriever = Arc::new(
            ...
            .with_weight_store(
                crate::retrieval_opt::RetrievalWeightStore::new(db.clone()),
            ),
        );
    ...
    /// Refuse the operation when the memory subsystem is disabled or in recovery.
    ///
    /// `pub` rather than `pub(crate)`: the tool handlers that gate on this now live
    /// in a different crate, so crate-private would silently push callers toward
    /// skipping the check.
    pub fn ensure_enabled(&self) -> MemoryResult<()> {
        if self.is_enabled() {
            Ok(())
        } else {
            Err(crate::error::MemoryError::Disabled)
        }
    }
    ...
    pub fn recovery_restore(&self, src: &str) -> MemoryResult<()> {
        ...
        let checker = crate::authority::StartupIntegrityChecker::new(self.db.clone());
    ...
    pub fn pending_enrichment_depth(&self) -> MemoryResult<u64> {
        SqliteEventStore::new(self.db.clone())
            .pending_count(crate::write_policy::slow::CONSUMER)
    }

    /// The ONE document-ingestion pipeline (M3). Records the item + chunks in the
    /// authority [`Library`](crate::library::Library) (SHA-256 dedup +
    ...
    pub fn ingest_document(
        ...
        for (idx, chunk) in crate::library::adaptive_chunk(content)
            .into_iter()
            .enumerate()
        {
            let mut cand = WriteCandidate::global(chunk);
            cand.source = crate::types::Source::Library {
                item: item_id,
                chunk: idx as u32,
            };
            ...
            for query in queries {
                ...
                for hit in result.hits {
                    ...
                    let truth_state = match hit.memory.state {
                        crate::types::MemoryState::Active => "Current",
                        crate::types::MemoryState::Promoted => "Confirmed",
                        crate::types::MemoryState::Archived => "Stale",
                        crate::types::MemoryState::Superseded => "Superseded",
                        crate::types::MemoryState::Forgotten => "Forgotten",
                        crate::types::MemoryState::Deleted => "Deleted",
                        _ => "Unknown",
                    };
                    let kind = match hit.memory.memory_type {
                        crate::types::MemoryType::Goal => "aggregate",
                        crate::types::MemoryType::Procedural => "evidence",
                        crate::types::MemoryType::WorldModel => "entity",
                        _ => "memory",
                    };
    ...
    /// [`authorize_read`](crate::policy::read_authorization::authorize_read)
    ...
    pub async fn search(
        ...
        use crate::authority::{SourceKind, SourceTrust};
    ...
    pub fn skill_view(&self, skill_id: &str) -> crate::integration::SkillMemoryView {
        crate::integration::SkillMemoryView::new(self.retriever.clone(), skill_id)
    }
    ...
    pub fn record_tool_outcome(
        ...
        if !crate::integration::outcome_is_salient(&content) {
        ...
        self.write_policy
            .submit(crate::integration::tool_outcome_candidate(
    ...
    pub fn record_capability(
        ...
        self.ensure_enabled()?;
        self.write_policy
            .submit(crate::integration::capability_candidate(
    ...
    /// [`crate::goals::GoalStore::planner_context`].
    pub fn goals(&self) -> crate::goals::GoalStore {
        crate::goals::GoalStore::new(self.db.clone())
    }

    /// The Planning Memory engine (plan-outcome learning, Priority 1).
    pub fn plans(&self) -> crate::planning::PlanStore {
        crate::planning::PlanStore::new(self.db.clone())
    }

    /// The Reasoning Memory engine (chains/hypotheses/counterexamples, Priority 2).
    pub fn reasoning(&self) -> crate::reasoning::ReasoningStore {
        crate::reasoning::ReasoningStore::new(self.db.clone())
    }

    /// The Dream Intelligence engine (procedure synthesis + goal optimization).
    pub fn dream_engine(&self) -> crate::dreaming::DreamEngine {
        crate::dreaming::DreamEngine::new(self.db.clone(), self.write_policy.clone())
    }

    /// The Research Memory engine (temporal retrieval, meta-memory, uncertainty
    /// propagation, Priority C).
    pub fn research(&self) -> crate::research::ResearchMemory {
        crate::research::ResearchMemory::new(self.db.clone())
    }

    /// The Causal Memory engine (cause→effect reasoning, counterfactuals).
    pub fn causal(&self) -> crate::causal::CausalMemory {
        crate::causal::CausalMemory::new(self.db.clone())
    }
    ...
    pub fn cold_start(&self) -> crate::cold_start::ColdStartConsent {
        crate::cold_start::ColdStartConsent::new(self.db.clone())
    }

    /// Read cold-start consent through the live memory master gate.
    pub fn cold_start_status(
        &self,
    ) -> MemoryResult<(bool, Vec<crate::cold_start::ScanSource>)> {
        self.ensure_enabled()?;
    ...
    pub fn set_cold_start_consent(
        &self,
        source: crate::cold_start::ScanSource,
        granted: bool,
    ) -> MemoryResult<()> {
        self.ensure_enabled()?;
        let consent = self.cold_start();
    ...
    pub fn cold_start_preview(
        ...
        root: Option<&str>,
        limit: usize,
    ) -> MemoryResult<Vec<crate::cold_start::ScanCandidate>> {
        self.ensure_enabled()?;
        crate::cold_start_scan::ColdStartScanner::new(self.db.clone())
    ...
    pub fn cold_start_import(
        &self,
        source: crate::cold_start::ScanSource,
        candidates: &[crate::cold_start::ScanCandidate],
    ) -> MemoryResult<usize> {
    ...
    pub fn cold_start_import_cancellable(
        ...
        for c in candidates {
            ...
            match source {
                ...
                crate::cold_start::ScanSource::Filesystem
                | crate::cold_start::ScanSource::Workspace => {
                    match std::fs::read_to_string(&c.path) {
                        ...
                        Ok(text)
                            if !text.trim().is_empty()
                                && !crate::cold_start_scan::content_has_secret(&text) =>
                        {
                        ...
                        _ => {
                            ...
                            cand.source = crate::types::Source::Import;
                ...
                crate::cold_start::ScanSource::Git => {
                ...
                crate::cold_start::ScanSource::Shell => {
    ...
    pub fn library(&self) -> crate::library::Library {
        crate::library::Library::new(self.db.clone())
    }

    /// The Knowledge-Graph Intelligence engine (centrality, communities, link
    /// prediction, transitive completion).
    pub fn graph_intelligence(&self) -> crate::graph_intel::GraphIntelligence {
        crate::graph_intel::GraphIntelligence::new(self.db.clone())
    }
    ...
    pub fn graph_search_entities(
        &self,
        query: &str,
    ) -> MemoryResult<Vec<crate::types::Entity>> {
        self.ensure_not_in_recovery_mode()?;
        self.ensure_enabled()?;
        use crate::stores::ports::GraphStore;
    ...
    pub fn graph_predict_links(
        ...
        entity: Uuid,
        limit: usize,
    ) -> MemoryResult<Vec<crate::graph_intel::LinkPrediction>> {
        self.ensure_not_in_recovery_mode()?;
        self.ensure_enabled()?;
    ...
    pub fn create_relationship(
        ...
        Err(crate::error::MemoryError::Internal(
    ...
    fn observability(&self) -> crate::observability::Observability {
        crate::observability::Observability::new(self.db.clone())
    }
    ...
    pub fn explain(
        &self,
        id: Uuid,
    ) -> MemoryResult<Option<crate::observability::MemoryExplanation>> {
        self.ensure_not_in_recovery_mode()?;
        self.ensure_enabled()?;
    ...
    pub fn memory_health_report(
        &self,
    ) -> MemoryResult<crate::observability::MemoryHealthReport> {
        self.ensure_not_in_recovery_mode()?;
        self.ensure_enabled()?;
    ...
    pub fn reasoning_replay(
        &self,
        session: &str,
    ) -> MemoryResult<Vec<crate::reasoning::ReasoningTrace>> {
        self.ensure_not_in_recovery_mode()?;
        self.ensure_enabled()?;
    ...
    pub(crate) fn entity_extraction(&self) -> crate::extraction::EntityExtractionPipeline {
        crate::extraction::EntityExtractionPipeline::new(self.db.clone())
    }
    ...
    pub(crate) fn retrieval_weights(&self) -> crate::retrieval_opt::RetrievalWeightStore {
        crate::retrieval_opt::RetrievalWeightStore::new(self.db.clone())
    }
    ...
    pub fn reinforce_retrieval(
        &self,
        class: crate::retriever::QueryClass,
        strategy: crate::retrieval_opt::Strategy,
    ) {
        if !self.is_enabled() {
            return;
        }
    ...
    pub(crate) fn active_learning(&self) -> crate::active_learning::ActiveLearning {
        crate::active_learning::ActiveLearning::new(self.db.clone())
    }
    ...
    pub fn record_knowledge_gap(&self, query: &str, domain: Option<&str>) -> MemoryResult<()> {
        self.ensure_not_in_recovery_mode()?;
        self.ensure_enabled()?;
        crate::knowledge_gap::KnowledgeGapEngine::new(self.db.clone())
    ...
    pub(crate) fn self_improvement(&self) -> crate::self_improvement::SelfImprovement {
        crate::self_improvement::SelfImprovement::new(self.db.clone())
    }
    ...
    pub fn reward_memories(&self, ids: &[Uuid], positive: bool) {
        ...
        let fb = crate::feedback::FeedbackService::new(self.db.clone());
        let signal = if positive {
            crate::feedback::FeedbackSignal::ThumbsUp
        } else {
            crate::feedback::FeedbackSignal::ThumbsDown
        };
        for id in ids {
    ...
    pub fn record_feedback(
        ...
        target_id: Uuid,
        target_kind: &str,
        signal: crate::feedback::FeedbackSignal,
        context: Option<&str>,
    ) -> MemoryResult<()> {
        self.ensure_not_in_recovery_mode()?;
        self.ensure_enabled()?;
    ...
    pub async fn correct(
        ...
        self.ensure_not_in_recovery_mode()?;
        self.ensure_enabled()?;
        use crate::stores::sqlite_search::index_fts_in_tx;
        use crate::types::{MemoryState, MemoryWorth, Sensitivity, VectorPayload};
        use rusqlite::params;
        ...
        let sensitivity =
            crate::sensitivity::resolve(&corrected_content, Some(&original.sensitivity));
        if sensitivity.class == Sensitivity::Secret {
            return Err(crate::error::SecurityError::SecretWrite.into());
        }
        ...
        corrected.content_hash = crate::ids::normalized_content_hash(&corrected_content);
        ...
        corrected.estimated_tokens = crate::governance::estimate_tokens(&corrected_content);
        ...
        let correction =
            crate::feedback::FeedbackSignal::Correction(corrected_content.clone());
        {
            ...
            tx.conn()
                ...
                .map_err(StorageError::Sqlite)?;
            crate::feedback::FeedbackService::new(self.db.clone()).record_in_tx(
                &mut tx,
                history.id,
                "memory",
                &correction,
    ...
    pub fn cognitive_scheduler(
        ...
        scheduler.register(Arc::new(crate::jobs::ActiveLearningJob::new(
            self.db.clone(),
        )));
        scheduler.register(Arc::new(crate::jobs::SelfImprovementJob::new(
            self.db.clone(),
        )));
        scheduler.register(Arc::new(crate::jobs::DreamJob::new(
            self.db.clone(),
            self.write_policy.clone(),
        )));
        scheduler.register(Arc::new(crate::jobs::EntityExtractionJob::new(
            self.db.clone(),
        )));
        scheduler
    }
    ...
    pub fn conversation(&self) -> crate::conversation::ConversationStore {
        crate::conversation::ConversationStore::new(self.db.clone())
    }
    ...
    fn truth(&self) -> crate::truth::TruthMaintenance {
        crate::truth::TruthMaintenance::new(self.db.clone(), self.relational_store())
    }

    fn entity_resolver(&self) -> crate::entity_resolution::EntityResolver {
        crate::entity_resolution::EntityResolver::new(
            self.db.clone(),
    ...
    pub fn forget(
        &self,
        scope: crate::lifecycle::ForgetScope,
        token: Option<&crate::lifecycle::LifecyclePreviewToken>,
    ) -> MemoryResult<usize> {
        self.ensure_not_in_recovery_mode()?;
        self.ensure_enabled()?;
    ...
    /// [`Lifecycle::shred_subject`](crate::lifecycle::Lifecycle::shred_subject)
    ...
    pub fn resolve_entities(
        ...
        entity_type: &str,
        alias: &str,
        alias_type: crate::entity_resolution::AliasType,
    ) -> MemoryResult<crate::entity_resolution::Resolution> {
        self.ensure_not_in_recovery_mode()?;
        self.ensure_enabled()?;
        let res = self
            .entity_resolver()
    ...
    pub async fn reflect(&self) -> MemoryResult<usize> {
        ...
        let (_sessions, accepted) = self
            .cognition(None)
            .consolidate_recent(
                crate::cognition::CognitionTrigger::Daily,
                8,
                &tokio_util::sync::CancellationToken::new(),
            )
            .await?;
    ...
    pub async fn consolidate(&self, session_id: Uuid) -> MemoryResult<usize> {
        ...
        let accepted = self
            ...
            .consolidate(
                session_id,
                crate::cognition::CognitionTrigger::SessionEnd,
            )
            .await?;
        self.notify_change(
            "reflection",
...
mod tests {
    ...
    use crate::types::{ModelVersion, Source};
    ...
    fn adapters_share_one_root_but_construct_distinct_callers() {
        ...
        use crate::model::{CallerContext, CallerOrigin, PolicyPartition};
    ...
    async fn change_events_fire_on_write_and_mutation() {
        ...
        sys.forget(crate::lifecycle::ForgetScope::Session(sess), None)
            .unwrap();
    ...
    async fn cold_start_preview_and_import_are_consent_gated() {
        use crate::cold_start::ScanSource;
        let sys =
            MemorySystem::open_for_test(MemoryConfig::default(), Arc::new(FakeEmbedder)).unwrap();
    ...
    async fn cognitive_report_aggregates_engines() {
        use crate::goals::{GoalStatus, NewGoal};
        let sys =
            MemorySystem::open_for_test(MemoryConfig::default(), Arc::new(FakeEmbedder)).unwrap();
        let sess = Uuid::now_v7();
    ...
    async fn public_api_facade_delegates_to_engines() {
        use crate::entity_resolution::{AliasType, Resolution};
    ...
    fn recovery_mode_blocks_all_durable_writes() {
        ...
        let err = sys
            .forget(crate::lifecycle::ForgetScope::Session(sess), None)
            .unwrap_err();
        assert!(
            matches!(err, MemoryError::InRecoveryMode { .. }),
        ...
        let err = sys
            .record_tool_outcome(
                sess,
                crate::types::Source::Tool("test-tool".into()),
                "tool blocked",
            )
            .unwrap_err();
        assert!(
    ...
    async fn recovery_mode_deep_check_and_health_still_work() {
        ...
        use crate::authority::integrity::CapabilityState;
        assert!(
            matches!(
                report.state,
        ...
        assert!(
            matches!(
                ...
                MemoryError::Recovery(
                    crate::error::RecoveryError::CannotExitWithoutVerifiedRestore
                )
            ),
