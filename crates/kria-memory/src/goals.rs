...
//! crate::authority::CommandCandidate::goal) is the typed
...
//! [`AuthorityCommandBus`](crate::authority::AuthorityCommandBus) today
...
///
/// **Superseded by** the canonical v2 [`crate::model::GoalStatus`]
...
///
/// **Superseded by** [`crate::model::Goal`] (`goals_v2`) +
/// [`crate::model::GoalProgress`] (canonical v2 goal). Retained as the
/// live goals persistence/read model until the F1.5 write cutover + F3
/// retrieval-on-v2; see the ledger in [`crate::model::legacy_mapping`]
/// (task F2.1.6).
...
impl GoalStore {
    ...
    pub fn decompose(&self, parent: Uuid, subgoals: &[String]) -> MemoryResult<Vec<Uuid>> {
        let parent_goal = self.get(parent)?.ok_or_else(|| {
            crate::error::MemoryError::Internal("decompose: parent goal not found".into())
        })?;
