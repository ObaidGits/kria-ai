...
//! reusable, higher-order memory. Unlike [`cognition`](crate::cognition)
...
use crate::types::{MemoryType, Source, WriteCandidate, WriteDecision};
...
impl DreamEngine {
    ...
    /// Policy. Reuses [`CausalMemory`](crate::causal::CausalMemory). A
    ...
    pub fn synthesize_rules(&self, effect: &str, max_new: usize) -> MemoryResult<usize> {
        let causal = crate::causal::CausalMemory::new(self.db.clone());
        let session = Self::dream_session();
        let mut accepted = 0usize;
    ...
    ///
    /// [`Cognition::record_run`]: crate::cognition::Cognition
...
mod tests {
    ...
    fn synthesizes_avoidance_rules_from_causal_failures() {
        let db = Arc::new(Database::open_in_memory().unwrap());
        let causal = crate::causal::CausalMemory::new(db.clone());
