...
use crate::model::{EntityId, GraphRevision, TruthState, UtcTimestamp};
...
impl GraphQueryProjector {
    ...
    pub fn build_navigation_facet(
        ...
        ProjectedNode {
            ...
            truth_state: crate::graph::projection::TruthStateSummary::bare(
                TruthState::Current,
            ),
            valid_time: None,
...
fn entity_row_to_node(row: &RawEntityRow, graph_revision: GraphRevision) -> ProjectedNode {
    ...
    let truth_state = match &row.staleness_class {
        Some(sc) => crate::graph::projection::TruthStateSummary::with_staleness(
            row.truth_state.clone(),
            sc.clone(),
        ),
        None => crate::graph::projection::TruthStateSummary::bare(row.truth_state.clone()),
    };
...
fn edge_row_to_edge(row: &RawEdgeRow, graph_revision: GraphRevision) -> ProjectedEdge {
    use crate::graph::projection::{EndpointSummary, TruthStateSummary};
...
fn hidden_endpoint_frontier(
    ...
    ProjectedNode {
        ...
        graph_revision,
        effective_policy: policy,
        truth_state: crate::graph::projection::TruthStateSummary::bare(TruthState::Current),
        valid_time: None,
        provenance: ProvenanceSummary {
            source_kind: None,
            actor_id: None,
...
mod tests {
    ...
    use crate::model::{EntityId, GraphRevision, TruthState};
