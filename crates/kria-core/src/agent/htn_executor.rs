...
pub enum PrerequisiteResult {
    ...
    Failed { prereq_id: String, reason: String },
...
impl GenericUiDismissal {
    ...
    pub async fn execute<T: ToolExecutor>(&mut self, tool_registry: &T) -> Result<bool, String> {
        // Step 1: Press Escape
        self.steps_executed.push(DismissalStep::PressEscape);
        let escape_result = tool_registry
            .execute("press_shortcut", &serde_json::json!({"keys": ["esc"]}))
            .await;
        ...
        let click_result = tool_registry
            .execute(
                "click_mouse",
                &serde_json::json!({
...
impl SelfCorrection {
    ...
    pub fn attempt_recovery(
        ...
        if spiral_check == SpiralCheckResult::SpiralDetected {
            tracing::error!("Recursive spiral detected for prerequisite: {}", prereq_id);
...
// ============================================================================
// Section 2: Verification Bridge
// ============================================================================

/// Convert legacy `VerificationType` to bounded `Verifiability`.
///
/// This bridge lets the `GuiExecutor` keep its existing `SubGoal.verify`
/// field while routing all verification through the single
/// `BoundedExecutionVerifier` authority.
fn verification_type_to_verifiability(
    vt: &VerificationType,
) -> crate::agent::execution_verifier::Verifiability {
    ...
    match vt {
        ...
        VerificationType::ScreenChanged { .. } => Verifiability::Unverifiable {
            reason: "ScreenChanged not yet mapped to bounded verifier".into(),
        },
        VerificationType::ElementsFound { .. } => Verifiability::Unverifiable {
            reason: "ElementsFound not yet mapped to bounded verifier".into(),
        },
...
pub struct GuiExecutor {
    /// Bounded execution verifier — single authority for all verification.
...
impl GuiExecutor {
    ...
    /// Construct with a custom verifier (e.g. production `BoundedExecutionVerifier`).
    ...
    pub async fn execute_workflow(
        ...
        for sub_goal in &workflow.sub_goals {
            ...
            match &target_window_lock {
                ...
                Some(expected) => {
                    ...
                    if !window_match && expected.pid != 0 {
                        ...
                        let should_halt = is_input_action
                            || consecutive_window_mismatches >= MAX_WINDOW_MISMATCHES;
            ...
            if sub_goal.action == "type_text" && self.kill_switch.modifier_was_pressed() {
                ...
                let _ = self
                    .tool_registry
                    .execute("release_all", &serde_json::json!({}))
                    .await;
            }
            ...
            if sub_goal.action == "click_element" {
                ...
                let next_is_type_text = workflow
                    .sub_goals
                    .iter()
            ...
            if !matches!(sub_goal.verify, VerificationType::None) {
                let verify_result = self
                    ...
                    .retry(|| async {
                        ...
                        if outcome.verified {
                            ...
                            Err(VerificationError::VerificationFailed(outcome.evidence))
                        }
                    })
                    .await;
                ...
                if let Err(e) = verify_result {
                    let error = format!(
                        ...
                        sub_goal.step, self.retries.max_attempts, e
                    );
                    tracing::error!(target: "gui_executor", task_id = %task_id, %error);
...
impl GuiWorkflowBuilder {
    ...
    pub fn add_abort_step(mut self, action: impl Into<String>, params: serde_json::Value) -> Self {
...
mod tests {
    ...
    fn test_absolute_cap_check() {
        ...
        assert!(matches!(
            runtime.check_absolute_cap(),
            CapCheckResult::TerminateTask(_)
        ));
    ...
    fn test_calculate_final_confidence() {
        ...
        let confidence =
            calculate_final_confidence(&chain, &runtime, ConfidenceSource::Operational);
    ...
    async fn test_gui_executor_budget_check() {
        use crate::tools::gui_automation::{GuiBackend, GuiError, Key, MouseButton, WindowInfo};
    ...
    fn test_pra_result_types() {
        ...
        let retry_result = PraResult::Retry {
            reason: "test".to_string(),
        };
        assert!(matches!(retry_result, PraResult::Retry { .. }));

        let abort_result = PraResult::Abort {
    ...
    fn test_anchor_verification_results() {
        ...
        let partial = AnchorVerification::PartialMatch {
            deviations: vec!["window mismatch".to_string()],
        };
        assert!(matches!(partial, AnchorVerification::PartialMatch { .. }));

        let failed = AnchorVerification::Failed {
    ...
    async fn test_e2e_gedit_launch_recovery() {
        ...
        match prereq_check {
            PrerequisiteResult::Failed { prereq_id, reason } => {
                ...
                let recovery = SelfCorrection::attempt_recovery(
                    &prereq_id,
                    &reason,
                    SpiralCheckResult::NewFailure,
                );
    ...
    async fn test_pra_loop_wiring_gedit_launch() {
        ...
        match &recovery_result {
            PraResult::InjectRecovery { prereq_id, subtree } => {
                ...
                for (i, sub_goal) in subtree.iter().enumerate() {
                    ...
                    assert!(
                        calls > i,
                        "ToolRegistry should have been invoked {} times",
                        i + 1
                    );
                }
            }
        ...
        assert_eq!(
            actions_consumed, 2,
            "Should have consumed 2 actions from budget"
        );
        ...
        assert_eq!(
            total_calls, 2,
            "ToolRegistry should have been invoked exactly twice"
        );
    ...
    async fn test_gated_sensing_to_visual_reasoner_wiring() {
        ...
        assert!(
            needs_resense,
            "Should require re-sense after forced invalidation"
        );
        ...
        assert_eq!(
            ocr_evidence.source,
            crate::agent::visual_reasoning::EvidenceSource::Ocr
        );
        ...
        let reasoning_result =
            visual_reasoner.reason_about_element(&initial_element, &ocr_evidence, &app_context);

        // Verify reasoning output
        match &reasoning_result {
            crate::agent::visual_reasoning::VisualReasoningOutput::ElementClassification(
                semantic,
                confidence,
            ) => {
                tracing::info!(
            ...
            crate::agent::visual_reasoning::VisualReasoningOutput::InsufficientConfidence => {
                tracing::info!(
                    "[WIRING TEST] VisualReasoner: Insufficient confidence (novel element)"
                );
            }
            _ => {}
        }
    ...
    fn test_spiral_safety_recursive_loop_prevention() {
        ...
        assert_eq!(
            first_check,
            SpiralCheckResult::NewFailure,
            "First failure should be NewFailure"
        );
        ...
        assert_eq!(
            second_check,
            SpiralCheckResult::SpiralDetected,
            "Second failure should trigger SpiralDetected"
        );
        ...
        match recovery_result {
            PraResult::HITLEscalation { reason } => {
                assert!(reason.contains("Recursive spiral detected"));
                tracing::info!(
                    "[SAFETY TEST] ✅ Spiral detected, HITL escalation triggered: {}",
                    reason
                );
            }
            _ => panic!(
                "Expected HITLEscalation on spiral, got {:?}",
                recovery_result
            ),
        }
        ...
        for i in 0..5 {
            ...
            tracing::info!(
                "[SAFETY TEST] Action {}: budget remaining = {}",
                i + 1,
                remaining
            );
        }
    ...
    async fn test_dry_run_audit_summary() {
        ...
        assert!(
            executor.prereq_checker.is_some(),
            "PrerequisiteChecker should be initialized"
        );
        assert!(
            executor.runtime_state.is_some(),
            "TaskRuntimeState should be initialized"
        );

        let state = executor.runtime_state.as_ref().unwrap();
        assert_eq!(
            state.action_budget_remaining, 25,
            "Budget should be initialized"
        );
        assert!(
            state.visited_failure_signatures.is_empty(),
            "Failure cache should start empty"
        );

        tracing::info!("✅ GuiExecutor PRA Loop initialized");
        tracing::info!(
            "✅ Budget: {} actions remaining",
            state.action_budget_remaining
        );
        tracing::info!(
            "✅ Spiral cache: {} signatures",
            state.visited_failure_signatures.len()
        );
        tracing::info!(
            "✅ Focus change flag: {}",
            state.os_focus_changed_since_last_sense
        );
    ...
    async fn test_absolute_cap_hard_termination() {
        ...
        let cap_check = executor
            .runtime_state
            .as_ref()
            .unwrap()
            .check_absolute_cap();
        assert!(
            matches!(cap_check, CapCheckResult::Continue),
            "should continue at 99"
        );
    ...
    async fn test_hard_window_switch_abort() {
        ...
        // Pre-flight: consume one get_active_window call so step 2 sees gedit and step 3 sees Windsurf
        let _ = verifier
            .verify(
        ...
        let mut executor =
            GuiExecutor::with_verifier(kill_switch, tool_executor, abort_executor, verifier);
