...
impl LedgerHandle {
    pub async fn create_job(
        ...
        rx.await
            .map_err(|_| anyhow::anyhow!("ledger actor dropped reply"))?
    }
    ...
    pub async fn get_phase_outputs(
        ...
        job_id: &str,
        cell_id: &str,
    ) -> anyhow::Result<Vec<PhaseArtifact>> {
        let (tx, rx) = oneshot::channel();
        self.tx
            .send(LedgerMsg::GetPhaseOutputs {
                job_id: job_id.into(),
                cell_id: cell_id.into(),
                reply: tx,
            })
...
impl LedgerActor {
        ...
        while let Some(msg) = self.rx.recv().await {
            match msg {
                ...
                LedgerMsg::UpdateHeartbeat {
                    ...
                    let _ = reply.send(self.update_heartbeat(
                        &job_id,
                        &cell_id,
                        heartbeat_ts,
                        &metrics,
    ...
    fn get_resume_point(&self, job_id: &str) -> anyhow::Result<Option<String>> {
        ...
        let result = stmt
            .query_row(rusqlite::params![job_id], |row| row.get::<_, String>(0))
            .optional()?;
        Ok(result)
    }
    ...
    fn get_phase_outputs(&self, job_id: &str, cell_id: &str) -> anyhow::Result<Vec<PhaseArtifact>> {
        let mut stmt = self
            .conn
            .prepare("SELECT outputs_json FROM ml_phases WHERE job_id=?1 AND cell_id=?2")?;
        let json: Option<String> = stmt
            .query_row(rusqlite::params![job_id, cell_id], |row| row.get(0))
            .optional()?;
        match json {
