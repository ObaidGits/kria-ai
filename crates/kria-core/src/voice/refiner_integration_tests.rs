...
mod tests {
    ...
    fn stale_generation_detection() {
        ...
        assert_ne!(
            ...
            "stale generation should be detectable"
        );
    }
    ...
    fn timeout_flag_detection() {
        ...
        assert!(
            ...
            "timed out refinement should have empty text"
        );
    }
    ...
    fn reconciliation_preserves_committed_on_reject() {
        ...
        if outcome.kind == ReconcileKind::Reject {
            assert_eq!(
                outcome.user_visible, outcome.ts_norm,
                "rejected refinement should preserve committed"
            );
        }
    }
    ...
    fn audio_accumulation_bounded() {
        ...
        assert_eq!(
            acc.len(),
            max_samples,
            "accumulation should be bounded to 480,000 samples"
        );
    }
