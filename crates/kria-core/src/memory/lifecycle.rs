...
//! those yet (see [`crate::memory::model::legacy_mapping`]). Two concrete
...
impl LifecyclePreviewToken {
    ...
    /// Encode as a compact string for use with [`crate::memory::authority::command::PreviewToken`].
...
impl Lifecycle {
    ...
    pub fn forget(
        ...
        for id in &ids {
            ...
            let tx_id = crate::memory::ids::new_id().to_string();
            tx.conn()
                .execute(
            ...
            let audit_id = crate::memory::ids::new_id().to_string();
            tx.conn()
                .execute(
            ...
            self.relational.enqueue_outbox(
                &mut tx,
                &crate::memory::types::OutboxEntry::delete(
                    *id,
                    crate::memory::types::IndexTarget::Fts,
                ),
            )?;
            self.relational.enqueue_outbox(
                &mut tx,
                &crate::memory::types::OutboxEntry::delete(
                    *id,
                    crate::memory::types::IndexTarget::LanceDb,
                ),
            )?;
    ...
    pub fn restore(&self, id: Uuid, token: Option<&LifecyclePreviewToken>) -> MemoryResult<()> {
        ...
        self.relational.enqueue_outbox(
            &mut tx,
            &crate::memory::types::OutboxEntry::upsert(
                id,
                crate::memory::types::IndexTarget::Fts,
                &memory.content_hash,
            ),
    ...
    pub async fn hard_delete(
        ...
        //    FTS in-txn, record governance rows, enqueue outbox deletes.
        {
            ...
            let grev_tx_id = crate::memory::ids::new_id().to_string();
            tx.conn()
                .execute(
                    "INSERT INTO graph_revisions(revision, base_revision, tx_id, committed_at, \
                     actor_id, policy_hash, change_count) \
    ...
    /// are all implemented.  See [`crate::memory::api::HealthReport`] for the
    ...
    /// [`crate::memory::api::CRYPTO_SHRED_CAPABILITY`]) can be changed from
...
mod tests {
    ...
    async fn preview_scope_limit_truncates_correctly() {
        ...
        let sid = crate::memory::ids::new_id();
        for i in 0..5usize {
            let ev = Event {
                id: crate::memory::ids::new_id(),
                hlc: crate::memory::ids::HlcGenerator::new().now(),
    ...
    async fn forgotten_memory_excluded_from_retriever() {
        ...
        let search_store = Arc::new(crate::memory::stores::SqliteSearchStore::new(db.clone()));
        ...
        let retriever = crate::memory::retriever::Retriever::new(
            rel.clone(),
            vectors.clone(),
            search_store,
            embedder,
        ...
        let before = retriever
            .search(
                "secret datum alpha",
                &crate::memory::retriever::RetrievalCtx::default(),
            )
            .await
            .unwrap();
        assert!(
    ...
    impl crate::memory::stores::ports::Embedder for FakeEmbedder {
        fn model_version(&self) -> crate::memory::types::ModelVersion {
            crate::memory::types::ModelVersion("fake_v1".into())
        }
        fn dim(&self) -> usize {
            self.dim
        }
        async fn embed(
            &self,
            texts: &[String],
        ) -> crate::memory::error::MemoryResult<Vec<Vec<f32>>> {
            Ok(texts
                .iter()
                .map(|t| {
        ...
        async fn health(&self) -> crate::memory::types::Availability {
            crate::memory::types::Availability::Up
        }
    }
}
