...
//! [`TxSemanticStore`]: crate::memory::authority::TxSemanticStore
//! [`DeferredSemanticStore`]: crate::memory::authority::DeferredSemanticStore
...
impl Maintenance {
            ...
            if !missing_fts.is_empty() {
                ...
                for &id in &missing_fts {
                    ...
                    self.relational.enqueue_outbox(
                        &mut tx,
                        &crate::memory::types::OutboxEntry::upsert(id, IndexTarget::Fts, hash),
                    )?;
                }
                tx.commit()?;
        ...
        // 7) Missing vector entries: active memories with no vector row.
        {
            ...
            if !missing_vecs.is_empty() {
                ...
                for &id in &missing_vecs {
                    ...
                    self.relational.enqueue_outbox(
                        &mut tx,
                        &crate::memory::types::OutboxEntry::upsert(id, IndexTarget::LanceDb, hash),
                    )?;
                }
                tx.commit()?;
    ...
    pub async fn relay(&self, target: IndexTarget, batch: usize) -> MemoryResult<usize> {
        ...
        let entry_map: HashMap<i64, &crate::memory::types::OutboxEntry> =
        ...
        for group in groups.values() {
            ...
            let apply_result: Result<(), crate::memory::error::MemoryError> =
...
mod tests {
    ...
    fn seed_memory_with_state(
        ...
        use crate::memory::stores::ports::RelationalStore;
        use crate::memory::types::Memory;

        let now = chrono::Utc::now();
        let id = new_id();
        let event_id = seed_event(db);
        let content_hash = crate::memory::ids::normalized_content_hash(content);
        let mem = Memory {
            id,
