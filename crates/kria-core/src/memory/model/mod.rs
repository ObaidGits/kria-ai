...
//! ([`crate::memory::db::encoding`]) and the id generator
//! ([`crate::memory::ids`]) — they do not re-implement UUID/timestamp rules.
//!
...
use crate::memory::db::encoding::{assert_rfc3339_utc, canonical_uuid};
...
pub(crate) fn encoding_err(msg: impl Into<String>) -> crate::memory::error::MemoryError {
...
macro_rules! uuid_newtype {
    ($(#[$meta:meta])* $name:ident, $what:literal) => {
        ...
        impl $name {
            ...
            pub fn new_v7() -> Self {
                Self(crate::memory::ids::new_id().to_string())
            }
        ...
        impl TryFrom<String> for $name {
            type Error = crate::memory::error::MemoryError;
...
/// UTC (design §14; validators in [`crate::memory::db::encoding`]).
