...
use crate::memory::manager::{MemoryManager, MemoryReader, MemoryTurnWrite};
use crate::memory::runtime_types::{
...
impl KriaMemoryRuntime {
    ...
    pub fn search_chunks(&self, query: &str, limit: usize) -> anyhow::Result<Vec<DocumentChunk>> {
        let Some(match_expr) = crate::memory::stores::fts5_query(query) else {
            return Ok(Vec::new());
        };
