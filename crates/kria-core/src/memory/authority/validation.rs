...
//!    gate ([`crate::memory::modes::evaluate`], MGR-035 AC4–AC7).
...
use crate::memory::model::{CallerOrigin, EventId, GraphRevision, IdempotencyKey};
use crate::memory::modes::{self, ModeWriteContext, ModeWriteDecision};
...
///
/// [`WritePolicy`]: crate::memory::write_policy::WritePolicy
...
impl<'r, R: ValidationReads + ?Sized> CommandValidator<'r, R> {
    ...
    // ── 3. mode ───────────────────────────────────────────────────────────
    /// Delegate to the deterministic mode gate ([`crate::memory::modes`]). Every
...
mod tests {
    use super::*;
    use crate::memory::authority::command::{Deadline, PreviewToken, SourceContext, SourceKind};
    use crate::memory::model::{CallerContext, InvocationId, PolicyPartition};
