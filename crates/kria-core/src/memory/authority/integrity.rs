...
//! [`MemorySystem::assemble`](crate::memory::api::MemorySystem::assemble) after
...
//! [`MemoryError`](crate::memory::error::MemoryError).
//!
...
use crate::memory::db::{migrations, Database};
...
impl From<StartupError> for crate::memory::error::MemoryError {
...
/// and accessed via [`crate::memory::authority::mod::AuthorityIntegrity::deep_check`].
///
    ...
    pub fn check_full_integrity(&self, report: &mut RecoveryCheckReport) {
        let result = self.db.with_read(|conn| {
            ...
            let mut stmt = conn
                ...
                .map_err(|e| crate::memory::error::StorageError::Sqlite(e))?;
