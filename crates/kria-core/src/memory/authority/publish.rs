...
//! ([`crate::memory::write_policy`]): "a full channel drops the *wake*, never
...
//! ([`crate::memory::api::MemorySystem::subscribe_changes`]). The adapter that
...
mod tests {
    use super::*;
    use crate::memory::authority::command::CommandEnvelope;
    use crate::memory::authority::command::{Deadline, SourceContext, SourceKind, SourceTrust};
    use crate::memory::authority::revision::{GraphChange, GraphChangeKind};
    use crate::memory::authority::transaction::{
        AuthorityTransaction, DeferredSemanticStore, SemanticOutcome, TxSemanticStore,
    };
    use crate::memory::authority::{CommandKind, OutboxPort, SqliteOutbox};
    ...
    fn for_committed_is_none_for_revision_neutral_command() {
        ...
        let record = txn
            .record_disposition_with_revision(
                &env,
                crate::memory::authority::AuditDisposition::Deferred,
                &neutral,
                &[],
                None,
            )
    ...
    fn lost_publication_is_recoverable_from_revisions_and_outbox() {
        ...
        for target in crate::memory::authority::PROJECTION_TARGETS {
    ...
    fn republishing_same_wake_does_not_duplicate_committed_truth() {
        ...
        let outbox = SqliteOutbox::new(db.clone());
        let pending_before: usize = crate::memory::authority::PROJECTION_TARGETS
            .iter()
        ...
        let pending_after: usize = crate::memory::authority::PROJECTION_TARGETS
            .iter()
            .map(|t| outbox.pending(t, 100).unwrap().len())
            .sum();
        assert_eq!(
