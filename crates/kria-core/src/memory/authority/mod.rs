...
//! root (`MemorySystem`, see [`crate::memory::api`]) exposes for the durable
...
//!   value objects from [`crate::memory::model`] so a raw unchecked string can
...
//!   the real governed write path ([`crate::memory::write_policy::WritePolicy`]
...
//! [`crate::memory::api::MemorySystem::compose`],
//! [`crate::memory::api::MemorySystem::outbox`], and
//! [`crate::memory::api::MemorySystem::integrity`]).
...
use crate::memory::db::{Database, ReconciliationReport};
...
/// [`WritePolicy`](crate::memory::write_policy::WritePolicy) is adapted to in
...
pub trait CommandPort: Send + Sync {
    ...
    /// reserved for storage/consistency failures — see [`crate::memory::error`]).
