...
//! ([`crate::memory::db::AuthorityTx`], the low-level SQL primitive) for the
...
//! [`AuthorityTx`]: crate::memory::db::AuthorityTx

use crate::memory::db::{AuthorityTx, Database};
use crate::memory::error::MemoryResult;
use crate::memory::model::{AuditId, EventId, GraphRevision};
...
impl TxSemanticStore for DeferredSemanticStore {
    fn apply(
        ...
        let change_kind = match env.kind() {
            crate::memory::authority::CommandKind::Observe => GraphChangeKind::Insert,
            crate::memory::authority::CommandKind::Correct => GraphChangeKind::Update,
            crate::memory::authority::CommandKind::Forget
            | crate::memory::authority::CommandKind::Restore => GraphChangeKind::State,
            crate::memory::authority::CommandKind::HardDelete => GraphChangeKind::Delete,
            // A declassification inserts a new audited provenance record.
            crate::memory::authority::CommandKind::Declassify => GraphChangeKind::Insert,
        };
...
mod tests {
    use super::*;
    use crate::memory::authority::command::{
        Deadline, PreviewToken, SourceContext, SourceKind, SourceTrust,
    };
    use crate::memory::authority::CommandKind;
    ...
    use crate::memory::authority::validation::{RejectionCode, RejectionReason};
    ...
    use crate::memory::authority::revision::{GraphChange, GraphChangeKind};
    ...
    use crate::memory::authority::validation::{
        validate_command, ValidationOutcome, ValidationReads,
    };
    use crate::memory::authority::SqliteValidationReads;
