...
use crate::memory::ids::{blake3_hex, HlcGenerator};
use crate::memory::modes::{evaluate, ModeManager, ModeWriteContext, ModeWriteDecision};
    ...
    pub fn submit(&self, cand: WriteCandidate) -> MemoryResult<WriteDecision> {
        ...
        if let Some(reason) = security::scan(&cand.content, &cand.source) {
            ...
            return Ok(WriteDecision::Rejected {
                reason: crate::memory::types::RejectReason::SecurityScan(reason),
            });
        }
