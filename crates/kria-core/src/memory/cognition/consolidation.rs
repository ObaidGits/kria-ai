...
use crate::memory::scheduler::{Priority, ResourceMonitor};
...
impl EpisodeManager {
    ...
    pub fn close_episode(
        ...
        let ep = self.get_episode(episode_id)?.ok_or_else(|| {
            crate::memory::error::MemoryError::Internal(
...
impl ConsolidationReadinessChecker {
    ...
    pub fn cap_confidence(source_kind: &str, confidence: f32) -> f32 {
        use crate::memory::cognition::REFLECTION_CONFIDENCE_CAP;
...
impl From<crate::memory::error::MemoryError> for DeriveRecordError {
    fn from(e: crate::memory::error::MemoryError) -> Self {
...
mod tests {
    ...
    async fn effective_policy_uses_most_restrictive_sensitivity() {
        ...
        let event_id = new_id().to_string();
        {
            use crate::memory::ids::new_id as gen_new_id;
            let hlc = format!(
                "{:016x}{:08x}",
                chrono::Utc::now().timestamp_millis() as u64,
