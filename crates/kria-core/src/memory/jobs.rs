...
use crate::memory::cognition::{Cognition, CognitionTrigger};
use crate::memory::error::MemoryResult;
use crate::memory::scheduler::{BackgroundJob, JobProfile, Priority};
...
pub struct ActiveLearningJob {
    active_learning: Arc<crate::memory::active_learning::ActiveLearning>,
    min_misses: u32,
    max_new: usize,
}

impl ActiveLearningJob {
    pub fn new(db: Arc<crate::memory::db::Database>) -> Self {
        Self {
            active_learning: Arc::new(crate::memory::active_learning::ActiveLearning::new(db)),
            min_misses: 3,
            max_new: 5,
        }
    }
...
pub struct SelfImprovementJob {
    engine: Arc<crate::memory::self_improvement::SelfImprovement>,
    max_new: usize,
}
...
impl SelfImprovementJob {
    pub fn new(db: Arc<crate::memory::db::Database>) -> Self {
        Self {
            engine: Arc::new(crate::memory::self_improvement::SelfImprovement::new(db)),
            max_new: 5,
        }
    }
}
...
pub struct EntityExtractionJob {
    pipeline: Arc<crate::memory::extraction::EntityExtractionPipeline>,
    batch: usize,
}
...
impl EntityExtractionJob {
    pub fn new(db: Arc<crate::memory::db::Database>) -> Self {
        Self {
            pipeline: Arc::new(crate::memory::extraction::EntityExtractionPipeline::new(db)),
            batch: 64,
        }
    }
}
...
pub struct DreamJob {
    engine: Arc<crate::memory::dreaming::DreamEngine>,
    max_procedures: usize,
}

impl DreamJob {
    pub fn new(
        db: Arc<crate::memory::db::Database>,
        write_policy: Arc<crate::memory::write_policy::WritePolicy>,
    ) -> Self {
        Self {
            engine: Arc::new(crate::memory::dreaming::DreamEngine::new(db, write_policy)),
            max_procedures: 8,
        }
    }
}
...
mod tests {
    use super::*;
    use crate::memory::cognition::CognitionTrigger;
    use crate::memory::db::Database;
    use crate::memory::modes::ModeManager;
    use crate::memory::scheduler::{CognitiveScheduler, StaticResourceMonitor};
