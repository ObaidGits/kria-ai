...
use crate::memory::retriever::RetrievalCtx;
use crate::memory::stores::ports::Embedder;
use crate::memory::types::{Availability, ModelVersion, WriteCandidate, WriteDecision};
...
fn it10_graph_neighbors_no_duplicates_within_cap() {
    ...
    use crate::memory::stores::sqlite_graph::SqliteGraphStore;
...
fn insert_rel_for_test(db: &Arc<crate::memory::db::Database>, source: Uuid, target: Uuid) {
...
async fn it12_crypto_shred_capability_is_unavailable() {
    ...
    assert_eq!(
        ...
        crate::memory::api::CRYPTO_SHRED_CAPABILITY
    );
...
async fn it13_health_report_has_no_content_fields() {
    ...
    assert_eq!(h.api_version, crate::memory::api::API_VERSION);
    assert!(
        h.memory_count >= 1,
