...
use kria_core::resource::authority::{
    Constraints, ConsumerId, DeviceId, HraService, LocalAuthority, PolicyProfile, PriorityClass,
...
fn task21_two_consumers_land_on_two_gpus() {
    ...
    let ra = LocalAuthority::bootstrap(
        &[(0, 12288), (1, 12288)],
        512,
        32768,
        &[],
    ...
    assert_ne!(
        l0.device, l1.device,
        "two big consumers must occupy two distinct GPUs"
    );
...
fn task21_no_overcommit_on_single_gpu() {
    ...
    match ra.request(&req(
        ...
        RaOutcome::Granted(l) => {
            assert!(matches!(l.device, DeviceId::CloudPool(_) | DeviceId::Cpu))
        }
...
fn task41_chaos_soak_holds_invariants() {
    ...
    assert!(
        svc.shadow_gate_passes(),
        "shadow comparator must remain clean over the soak"
    );
    assert!(
        svc.metrics().foreground_invariant_ok(),
        "no non-emergency foreground interrupts"
    );
}
...
async fn phaseb_llm_and_image_co_reside_then_fg_preempts() {
    ...
    let llm = cor
        .acquire(
            &cor_req(ConsumerId::Llm, PriorityClass::InteractiveFg, 4000, "llm"),
            ResidencyTarget::Hot,
        )
        .await
    ...
    let img = cor
        .acquire(
            &cor_req(ConsumerId::Image, PriorityClass::Batch, 4000, "img"),
            ResidencyTarget::Hot,
        )
        .await
        .expect("image co-resident grant");
    assert!(
        llm.is_valid() && img.is_valid(),
        "both co-resident on one GPU"
    );
    ...
    img.release().await;
    assert_eq!(
        cor.resident_count().await,
        0,
        "reservations drain — no leak"
    );
}
...
async fn phaseb_foreground_preempts_background_on_full_gpu() {
    ...
    let img = cor
        .acquire(
            &cor_req(ConsumerId::Image, PriorityClass::Batch, 6000, "img"),
            ResidencyTarget::Hot,
        )
        .await
        .expect("image grant");
    let llm = cor
        .acquire(
            &cor_req(ConsumerId::Llm, PriorityClass::InteractiveFg, 6000, "llm"),
            ResidencyTarget::Hot,
        )
        .await
    ...
    assert!(
        !img.is_valid(),
        "preempted background lease is cooperatively revoked"
    );
