...
use kria_core::resource::authority::{
    CoResidencyManager, Constraints, ConsumerId, LocalAuthority, ModelDescriptor, ModelHealth,
...
async fn mgr(models: usize) -> Arc<CoResidencyManager> {
    let auth = Arc::new(LocalAuthority::bootstrap(
        &[(0, 24576)],
        512,
        65536,
        &[],
...
async fn bench_admission_acquire_release_latency() {
    ...
    for _ in 0..200 {
        if let Ok(l) = cor
            .acquire(&req(PriorityClass::Batch, 1000, "m0"), ResidencyTarget::Hot)
            .await
        {
            l.release().await;
        }
    ...
    for i in 0..n {
        ...
        if let Ok(l) = cor
            .acquire(
                &req(PriorityClass::Batch, 1500, &model),
                ResidencyTarget::Hot,
            )
            .await
        {
    ...
    let avg: u128 = if samples.is_empty() {
        0
    } else {
...
async fn bench_dedup_hit_rate_and_latency() {
    ...
    let anchor = cor
        .acquire(
            &req(PriorityClass::InteractiveFg, 2000, "m0"),
            ResidencyTarget::Hot,
        )
        .await
        .unwrap();
    ...
    println!(
        "[HRA bench] dedup: hits={} hit_rate={hit_rate:.3} p99={p99}us",
        m.dedup_hits
    );
...
async fn bench_preemption_latency() {
    ...
    for _ in 0..iters {
        let bg = cor
            .acquire(&req(PriorityClass::Batch, 6000, "m0"), ResidencyTarget::Hot)
            .await
            .unwrap();
        let t = Instant::now();
        let fg = cor
            .acquire(
                &req(PriorityClass::InteractiveFg, 6000, "m1"),
                ResidencyTarget::Hot,
            )
            .await
            .unwrap();
