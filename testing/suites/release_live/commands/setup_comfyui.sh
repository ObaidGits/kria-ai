...
#
# Usage:
#   bash testing/suites/release_live/commands/setup_comfyui.sh
#
...
if [[ -z "${PYTHON}" ]]; then
    die "Could not find a usable Python (need 3.10, 3.11, or 3.12).
        ...
        Or, to force-use your existing Python 3.13 (may fail on torchvision):
          FORCE_PY313=1 bash testing/suites/release_live/commands/setup_comfyui.sh"
fi
    ...
    if install_torch_from_index "${CUDA_VERSION}"; then
        ...
        if [[ "${CUDA_VERSION}" != "cu118" ]] && install_torch_from_index "cu118"; then
            ...
            "${VENV_PIP}" install --upgrade torch torchvision || \
                die "PyTorch install failed completely. Try:  REBUILD_VENV=1 COMFY_PYTHON=\$(which python3.12) bash testing/suites/release_live/commands/setup_comfyui.sh"
            CUDA_VERSION="cpu"
        fi
    fi
fi
...
if [[ "${CUDA_OK}" == "yes" ]]; then
    ...
    warn "Try:  CUDA_VERSION=cu118 REBUILD_VENV=1 bash testing/suites/release_live/commands/setup_comfyui.sh"
else
