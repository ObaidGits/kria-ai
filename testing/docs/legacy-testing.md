...
> Central testing note: the `testing/` spine is now the preferred orchestration
> entrypoint. Framework-native Rust and Vitest paths still exist, while shell
> eval/release command bodies live under `testing/suites/*/commands`. Use:
>
> ```bash
...
> ```
>
> This document remains a historical/framework-native reference after the
> centralized testing migration.
...
| **Playwright API tests** | Playwright (TS) | `testing/suites/playwright/tests/*.api.spec.ts` | HTTP contract validation, WS integration |
| **Playwright E2E tests** | Playwright (TS) | `testing/suites/playwright/tests/*.e2e.spec.ts` | Full browser user journeys (chat, settings, voice) |
...
| **Runner Release** | `./testing/run.sh scenario release_live.scripts_run_release_test_gate --include-destructive --include-slow` | Production gate profile with strict thresholds + behavior goldens |
...
All Playwright tests use page objects in `testing/suites/playwright/pages/`:
