...
impl VitsStreamingModel {
    ...
    fn infer_with_values(&self, input_phonemes: Vec<i64>) -> PiperAudioResult {
        ...
        let mut decoder_model = self.decoder_model.lock().unwrap();
        let audio = encoder_output.infer_decoder(&mut decoder_model)?;
        let inference_ms = timer.elapsed().as_millis() as f32;
        Ok(Audio::new(
            audio,
            self.config.audio.sample_rate as usize,
    ...
    fn infer_encoder(&self, input_phonemes: Vec<i64>) -> PiperResult<EncoderOutputs> {
        ...
        let run_result = session
            ...
            .map_err(|e| PiperError::OperationError(format!("Failed to run model inference. Error: {}", e)))?;
        let encoded = EncoderOutputs::from_values(run_result)?;
        Ok(encoded)
    }
}
...
impl EncoderOutputs {
    ...
    fn from_values(values: SessionOutputs) -> PiperResult<Self> {
        let z = {
            ...
            let z_shape = z_t.0.to_ixdyn();
            Array::from_shape_vec(z_shape, z_t.1.to_vec()).map_err(|e| {
                PiperError::OperationError(format!("Failed to shape encoder z tensor: {}", e))
            })?
        };
        let y_mask = {
            ...
            let y_mask_shape = y_mask_t.0.to_ixdyn();
            Array::from_shape_vec(y_mask_shape, y_mask_t.1.to_vec()).map_err(|e| {
                PiperError::OperationError(format!(
                    "Failed to shape encoder y_mask tensor: {}",
                    e
                ))
            })?
        };
        let p_duration = if values.contains_key("p_duration") {
            ...
            let p_duration_shape = p_duration_t.0.to_ixdyn();
            Some(
                Array::from_shape_vec(p_duration_shape, p_duration_t.1.to_vec()).map_err(|e| {
                    PiperError::OperationError(format!(
                        "Failed to shape encoder p_duration tensor: {}",
                        e
                    ))
                })?,
            )
        ...
        let g = if values.contains_key("g") {
            ...
            let g_shape = g_t.0.to_ixdyn();
            Array::from_shape_vec(g_shape, g_t.1.to_vec()).map_err(|e| {
                PiperError::OperationError(format!("Failed to shape encoder g tensor: {}", e))
            })?
        } else {
...
impl SpeechStreamer {
    ...
    fn synthesize_chunk(
        ...
        let audio = {
            ...
            let audio_shape = audio_t.0.to_ixdyn();
            let audio_arr = Array::from_shape_vec(audio_shape, audio_t.1.to_vec()).map_err(|e| {
                PiperError::OperationError(format!("Failed to shape decoder audio tensor: {}", e))
            })?;
...
impl Iterator for SpeechStreamer {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.one_shot {
            ...
            let mut decoder_model = self.decoder_model.lock().unwrap();
            Some(self.encoder_outputs.infer_decoder(&mut decoder_model))
        } else {
