...
The model target is **`all-MiniLM-L6-v2`, 384 dimensions**, served through ONNX Runtime (the `ort` crate) with a WordPiece `tokenizers` pipeline — not the `fastembed` crate, which is used only by the separate intent-routing embedder. Release does not rely on the current `minilm_v1` label or comments for identity/license. `models/manifest` must pin canonical model ID, source URL/repository revision, artifact and tokenizer checksums, reviewed license disposition, ONNX Runtime (`ort`)/tokenizer runtime versions, maximum tokenization contract, pooling, and L2 normalization. Vectors are exactly 384 finite IEEE-754 `f32` values serialized little-endian (`1536` bytes). NaN/Inf, wrong byte count, zero norm, model hash, tokenizer hash, or dimension rejects the partition.
