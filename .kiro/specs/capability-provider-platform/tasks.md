...
- [x] **M12 Single-pipeline migration (Option A, LOCKED)** — DONE. KRIA now has ONE execution architecture (CPP). Legacy chat/OpenClaw execution + routing + permission + marketplace + CIL are removed. Staged, build-green each step:
  - [x] **Stage 1 — CPP chat dispatcher (core):** `crate::tools::capability_dispatch::CapabilityDispatchHandler` — the single agent/chat execution entry point: `query → CapabilityPlatform.discover → capability::permission (one engine + one durable GrantStore) → CapabilityPlatform.execute`. NL→typed-args via the neutral `openclaw::arg_gen`. 2 unit tests green (GREEN capability runs without prompt; no-match is honest).
  - [x] **Stage 2 — chat wired to CPP:** `kria-desktop/commands/runtime.rs` registers the dispatcher for the `openclaw` tool (overwrites the legacy `SemanticOpenClawHandler` by name; `list_installed_skills` kept). Reuses the shared embedding model, the OpenClaw provider (with marketplace lifecycle), and the durable `cpp_grants.db`. `cargo build -p kria-desktop` clean. **Chat capability execution now flows through the one platform.**
  - [ ] **Stage 3 — unify marketplace UI:** point the Marketplace surface at the remote provider catalog (`cpp_recommend`/`clawhub_fetch_remote_skills`); installed-skills becomes a separate view.
  - [ ] **Stage 4 — inline approval on the chat path:** map a `Prompt` decision to the desktop approval modal round-trip (today it returns an honest "approve in Capabilities panel" message; durable grant then makes reuse silent).
  - [-] **Stage 5 — delete legacy:**
    - [x] Batch A: unwire the legacy handler — `runtime.rs` no longer calls `register_into_tool_registry(_with_cil)`; `openclaw` + `list_installed_skills` are served by CPP (`CapabilityDispatchHandler` + `CapabilityListHandler`). Both legacy init registration methods now have ZERO production callers (grep-confirmed). `cargo build -p kria-desktop` clean.
    - [x] Batch B: **`handler.rs` DELETED** (`SemanticOpenClawHandler`, `register_semantic_openclaw`, `register_skill`, `ListInstalledSkills`, `execute_semantic` + its ~700-line test module). Reusable `build_runtime_registry` relocated to the neutral `openclaw::runtime` module; all callers updated. Deleted the 3 legacy init registration methods (`register_into_tool_registry`, `_with_llm`, `_with_cil`). Removed the legacy-handler tests (`execute_e2e::r4_4_*`, `trust_revocation` community-network + source-tripwire, `openclaw_live_docker` NL-e2e). `build_cil_facade` now dead (removed with cil in Batch F). kria-core + kria-eval + kria-desktop all compile (lib + tests); `capability_dispatch` tests green.
    - [x] Batch D: **`openclaw::perm` DELETED** (duplicate grant store + permission engine gone). Desktop `openclaw_list_grants`/`openclaw_revoke_grant` migrated to `capability::grants` + `capability::permission` over the ONE `cpp_grants.db` (shared with the dispatcher + Capabilities panel). kria-core + kria-desktop compile green.
    - [x] Batch F: **`openclaw::cil` DELETED** (whole module ~15 files) + `init::build_cil_facade` + the `OpenClawConfig.cil` field + `openclaw_icp_enabled` flag + `pub use cil::{...}` re-exports. Desktop cil commands (`openclaw_recommend_skills`/`openclaw_capability_manager`/`openclaw_capability_graph`) rewritten cil-free (delegate to CPP / registry-only honest results). Eval cil suites removed (`telemetry_completeness` deleted; `honesty_sweep` trimmed to keep `honesty_ledger`). kria-core + kria-desktop + kria-eval green.
    - [x] Batch C: **`SemanticSkillRouter` DELETED** (`semantic_router.rs` + `semantic_router_tests.rs` + mod re-exports). Eval routing tests migrated to registry-based checks (`skill_management` hot enable/disable via `get_enabled_skills`, `performance_budgets` → registry-lookup-only) or removed (`execute_e2e::r4_no_match`, covered by `capability_dispatch::dispatcher_no_match_is_honest`).
    - [x] Batch E: **`ApprovalCache` DELETED** (`approval.rs` removed). Its one reusable util `compute_hash` extracted to the tiny neutral `openclaw::cap_hash`; `runtime/docker.rs` updated. Legacy widening tests removed (covered by `capability::permission` Property 7). Only `capability::permission` + `capability::grants` remain.
    - **Result:** grep-confirmed ZERO production-src references to `SemanticOpenClawHandler`, `SemanticSkillRouter`, `openclaw::perm`, `openclaw::cil`, `ApprovalCache`, or `openclaw_icp_enabled`. 110 capability lib tests green; all three crates + tests compile.
  - [ ] **Stage 6 — default-on + real desktop drive:** flip `[capability].enabled` default on; drive real chat/marketplace/permission end-to-end; verify persistence across restart.
