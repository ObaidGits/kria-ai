...
- [ ] 5. Clear failure reporting — root cause, not opaque flapping (Issue #12)
  - [ ] 5.1 In `mod.rs` (workflow loop abort/block paths) + `recovery.rs`, map the bounded-guard stop reasons (`"screen state repeated N times without progress"`, `"re-observe budget reached"`, `"the resolved target is no longer present"`) to the UPSTREAM root cause and surface THAT as the user-facing reply: `target_not_found` / `app_not_focused` / `field_not_resolvable_vision_unavailable` / `needs_clarification` / `load_not_ready`. Keep the bounded flapping/runaway guard exactly as-is; only the REPORTED reason changes. Carry a structured `blocker_kind` + sanitized `blocker_detail` in the stop event
  - [ ] 5.2 Flag `gui_cog_clear_failure` (env `KRIA_GUI_COG_CLEAR_FAILURE`, default-ON after gate; falsy = prior messaging byte-for-byte)
  - [ ] 5.3 CI tests: a no-candidate resolution stop reports `target_not_found` (not "screen repeated"); an unresolvable-field stop maps to `field_not_resolvable`; an app-not-active stop maps to `app_not_focused`; each root cause maps to its message; flag-OFF parity (event bytes unchanged)
  - [ ] 5.4 `cargo build` both crates clean; lib + workflow_runtime + recovery suites green
  - [ ] 5.5 LIVE gate: a deliberately-unresolvable prompt (e.g. "click the Foobar button" with no such control) returns an actionable root-cause reason (not "screen repeated N times"); 0 leak; record the reply
  - **Done-when:** 5.1 mapping in place + 5.3 tests green + 5.5 live (actionable reason)
  - _Flag: `gui_cog_clear_failure`_  _Requirements: 12_

- [ ] 6. Smarter bounded recovery (Issue #13)
  - [ ] 6.1 In `recovery.rs` + the workflow recovery loop (`mod.rs` `run_recovery_loop`/`assess_recovery`): for TRANSIENT, IDEMPOTENT failures (`load_not_ready`, `focus_lost`, navigation not yet rendered) do a BOUNDED recovery — re-activate the target window via the extension and/or wait-then-reobserve — capped by the Task-1 runaway caps. NEVER retry a non-idempotent/destructive action (gate on `default_idempotent_for` / `primitive_tier`). Build on the Task-2 navigation-wait as the canonical "wait-then-reobserve" primitive
  - [ ] 6.2 Record the `RecoveryDecision` (kind, idempotent?, bounded-retry count, outcome) as additive telemetry; never auto-retry a risky action; a retried step still goes through the safety gate
  - [ ] 6.3 Flag `gui_cog_smart_recovery` (env `KRIA_GUI_COG_SMART_RECOVERY`, default-ON after gate; falsy = prior stop-on-fail byte-for-byte)
  - [ ] 6.4 CI tests (`gui_cognition_recovery_tests`): idempotent transient failure → bounded re-activate/wait then success; non-idempotent failure → NO retry (stops); cap respected (never exceeds `effective_max_reobserve`); flag-OFF parity
  - [ ] 6.5 `cargo build` both crates clean; lib + recovery suites green
  - [ ] 6.6 LIVE gate: a focus-race / load-not-ready prompt (e.g. open a slow app then act) recovers-and-completes OR stops with a clear Task-5 reason (no flap); 0 leak; record
  - **Done-when:** 6.1 bounded recovery in place + 6.4 tests green + 6.6 live (recovers or clean stop, no risky retry)
