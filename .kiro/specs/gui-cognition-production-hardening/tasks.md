...
- **Status:** Tasks 1–6 DONE + live-verified. Tasks 8, 9, 10, 11, 12 DONE + CI + LIVE-verified (Task 12
  probe-latency verified; turn-level p50 now resolvable via Task 11's local planner now that the resident
  model is served). Tasks 8 + 11 are now LIVE-verified end-to-end: the resident `Qwen2.5-VL-7B-Instruct`
  `llama-server` is served on a STABLE port (`KRIA_LLAMA_SERVER_PORT=8090`) with the mmproj clip kept in
  system RAM (`--no-mmproj-offload`, the fix for the "exited before reporting listening port" OOM on the
  6 GB GPU); ONE resident model serves BOTH the planner (Task 11) and the `vl7b` vision sidecar (Task 8),
  sequential. Remaining: **Tasks 7 + 13 LIVE gates** are still deferred — Task 7 needs the abs-pointer
  executor click routing + a session re-login (device re-registration), and Task 13 needs disabling the
  GNOME extension + re-login. Task 8's click-on-vision-resolved-target half is part of Task 7. Every fix is
  flag-gated (`gui_cog_*` / `KRIA_*` env, default-ON, falsy=rollback, flag-OFF byte-for-byte) with CI tests;
