...
- [x] 7. Wayland absolute pointer / coordinate click (Issue #4) — **CODE+CI DONE incl. executor routing (daemon EV_ABS + ClickAbs IPC + backend + ClickControl→abs glue + tool param; 14 CI tests); EV_ABS device registered post-relogin. LIVE click-lands is bounded by the bounds SOURCE on this 6GB/a11y-limited box (honest — not a code defect; see 7.6).**
  ...
  - [x] 7.2 IPC plumbing + executor routing wired (glue COMPLETE): `kria-core/src/tools/gui_automation.rs` — `IpcRequest::ClickAbs` (serializes to `cmd:"click_abs"`), `GuiBackend::click_mouse_abs` (default-errors; `YdotoolBackend` override sends `ClickAbs`, rejecting out-of-[0,65535]); the `click_mouse` tool now accepts `absolute:true` → `click_mouse_abs`. `executor.rs` — `GuiActionRequest.abs_click: Option<GuiAbsClick>` (`#[serde(default)]` → flag-OFF/old-payload byte-for-byte), plus `gui_abs_pointer_enabled`, `gui_pixel_to_abs`, `desktop_physical_extent`, `abs_click_for_target` (normalize a TRUSTED target center to [0,65535] from the observed `monitor_layout`; `None` when no layout/bounds — never invents a coord). `mod.rs` — a ClickControl with a resolved target carrying trusted bounds + abs-pointer flag ON computes `abs_click`. `gui_cognition.rs` (desktop) — the click arm dispatches `abs_click` via `click_mouse {absolute:true}` and falls back HONESTLY to `click_ui_element` on failure (no silent no-op). No-bounds → honest block (Task 5 reason) preserved
  - [x] 7.6 LIVE gate (real GNOME Shell 46 Wayland, **re-login done** — EV_ABS device registered, `uinput_available:true`, `click_supported:true`): the abs-pointer PATH is wired end-to-end and unit-proven; the daemon performs `ClickAbs` only with a valid heartbeat (dead-man switch). HONEST FINDING — the live "click LANDS on a control" cannot be completed on THIS box because no TRUSTED control bounds are obtainable in interactive time: (a) the AT-SPI snapshot exposes per-node extents only for the FOCUSED element, not arbitrary click targets, and the tested apps (Tauri webview, gnome-calculator) reported control `bounds:null`; (b) real VL-7B full-desktop grounding on the shared 6 GB GPU is too slow (~50 s, competing with the planner) and returned 0 detections on the cluttered desktop (verified: `[VISION] found 0 elements, took 50011.9ms`). With no trusted target bounds the abs-click CORRECTLY does not fire — `abs_click=None` → AT-SPI fallback / honest block, NEVER an invented coordinate. So the bottleneck is the bounds SOURCE (a11y/vision on this hardware), not the abs-click code, which is CI-proven and will fire when trusted bounds are available (faster/dedicated vision GPU, or an AT-SPI-rich app exposing extents). 0 destructive; no fabricated numbers
  ...
  - **Done-when:** 7.1 EV_ABS click path implemented + 7.4 event-shape tests green + executor abs-click glue wired (7.2) + CI-proven. Live click-lands is gated on a trusted bounds source (a11y extents or fast vision) which this 6GB/a11y-limited box does not provide in interactive time (honest — abs-click code is complete + tested, fires when bounds are available)
  - _Flag: `gui_cog_abs_pointer`_  _Requirements: 4_

- [x] 8. Real visual perception — replace dummy vision with VL-7B (Issue #1) — **DONE + CI + LIVE-VERIFIED end-to-end (real VL-7B grounding `degraded=false`, desktop→sidecar(vl7b)→resident model on stable port 8090). Click-on-vision-resolved-target half pairs with Task 7 (abs pointer, re-login-deferred).**
  ...
  - [x] 8.6 LIVE gate (real GNOME Wayland; resident VL-7B served on the GPU): **VERIFIED end-to-end.** Setup that made it work on the 6 GB GPU: (a) the mmproj OOM fix — `server_manager.rs` now passes `--no-mmproj-offload` so the ~2 GB clip projector lives in system RAM (was aborting llama-server before the listening port → the "exited before reporting listening port" rollback); (b) `KRIA_LLAMA_SERVER_PORT=8090` gives the resident `Qwen2.5-VL-7B-Instruct` a STABLE endpoint; (c) the desktop launches the vision sidecar with `KRIA_VISION_MODEL=vl7b` + `KRIA_VL_ENDPOINT=http://127.0.0.1:8090/v1/chat/completions` (inherited by the orchestrator-spawned child), so ONE resident model serves BOTH the planner and the vision grounding (sequential). Proof through the FULL production chain (desktop → sidecar `/parse_screen` → resident model): a labeled UI image → `model=qwen2.5-vl-7b`, **`degraded=false`**, real detections `button "Save"`, `button "Cancel"`, `input "Filename"` (the dummy stub returns `degraded=true` + synthetic boxes — clearly distinguished). Latency ~11 s per full-screen grounding on the shared 6 GB GPU; bbox precision is approximate (text-region boxes) but type+label detection is real + authoritative. HONEST REMAINDER: the click-ON-a-vision-resolved-target half (resolve unique target from these detections → click + verify) is gated on **Task 7** (abs-pointer executor routing + session re-login), still deferred — the VISION replacement this task owns is done + live-proven; 0 leak; no fabricated numbers
...
- [x] 11. Local grammar planner rung (Issue #2) — **DONE + CI + LIVE-VERIFIED (local grammar planner produces schema-valid plans on the resident `qwen2.5-vl-7b` AND executes live)**
  ...
  - [x] 11.6 LIVE gate (real GNOME Wayland; resident VL-7B served on the stable port 8090): **VERIFIED.** A `gui_cognition` `execute_live` turn planned on the LOCAL model — `planner.mode=llm_schema`, `planner.model=qwen2.5-vl-7b`, `capability.structured_mode=grammar`, `supports_grammar=true`, `validation_status=valid`, `plan_status=valid` (warmup observe conf 0.94; "scroll down on the current window" conf 0.82). The local grammar-constrained plan then EXECUTED live: `execution.action_type=Scroll`, `status=completed`, verification present, reply "Workflow completed 2 verified step(s) safely, one bound action at a time." So the structured/grammar planner runs against the local `llama-server` (no cloud round-trip needed) and yields schema-valid, executable plans. 0 destructive; no fabricated numbers. (The specific cloud-reject→`ladder_rung=local_grammar` fallback transition is CI-verified by `gui_cognition_llm_planner_tests`; live, the local backend is the routed grammar-capable planner and produced the valid plans directly.)
...
- [x] 13. Reduce single-point GNOME-extension dependency + graceful degrade (Issue #11) — **DONE + CI + LIVE-VERIFIED (extension toggled live via `gnome-extensions disable/enable`; honest degrade then restore, no crash/silent-fail)**
  ...
  - [x] 13.6 LIVE gate (real GNOME Shell 46 Wayland): **VERIFIED — no re-login needed** (toggled the extension live via the `gnome-extensions` CLI). Baseline (extension ACTIVE): `gui_backend_status = {extension_available:true, capture_available:true, activate_available:true, preferred_focus_backend:"gnome_bridge", capability_notices:[]}`. After `gnome-extensions disable kria-active-window@kria.ai`: **honest degrade** — `{extension_available:false, capture_available:false, preferred_focus_backend:"uinput_alt_tab", capability_notices:["GNOME window extension unavailable: window activation/capture is degraded; using the best available fallback","screen capture unavailable (no extension or portal backend)"]}`, `can_execute_actions` STILL `true` (uinput keyboard path intact) — graceful, NO crash, NO silent failure. After `gnome-extensions enable`: **fully restored** (`extension_available:true`, `preferred_focus_backend:"gnome_bridge"`, no notices). 0 destructive; no fabricated values
...
- **Status:** ALL 13 tasks implemented + CI-green; 11 of 13 fully LIVE-verified (1–6, 8, 9, 10, 11, 12,
  13). The resident `Qwen2.5-VL-7B-Instruct` `llama-server` is served on a STABLE port
  (`KRIA_LLAMA_SERVER_PORT=8090`) with the mmproj clip kept in system RAM (`--no-mmproj-offload`, the fix
  for the "exited before reporting listening port" OOM on the 6 GB GPU); ONE resident model serves BOTH the
  planner (Task 11) and the `vl7b` vision sidecar (Task 8). Task 13 was live-verified by toggling the GNOME
  extension live (`gnome-extensions disable/enable`, no re-login) — honest degrade then restore. **Task 7**
  is CODE-COMPLETE + CI-green (daemon EV_ABS + IPC + backend + executor abs-click glue + tool param, 14
  tests) with the EV_ABS device registered after re-login; its live "click-LANDS" gate is the one item not
  closed on THIS hardware, bounded by the trusted-bounds SOURCE (the AT-SPI snapshot exposes extents only
  for the focused element, and real VL-7B full-desktop grounding is ~50 s/0-detections on the shared 6 GB
  GPU) — the abs-click correctly never invents a coordinate, so it no-ops to the honest AT-SPI/​block path
  here and will fire when a faster/dedicated vision GPU or an AT-SPI-rich target supplies bounds. Every fix
  is flag-gated (`gui_cog_*` / `KRIA_*` env, default-ON, falsy=rollback, flag-OFF byte-for-byte) with CI
  tests; no fabricated live numbers.
