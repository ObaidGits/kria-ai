...
import { NavigationRail as Dock, SPACE_GROUP, spaceOutcome } from "./NavigationRail";
...
describe("Dock — presentation-only grouping/emphasis (task 7.6)", () => {
  ...
  it("emphasizes only the primary destination (Converse)", async () => {
    ...
    const primary = buttons.filter((b) =>
      b.classList.contains("kria-navrail__button--primary"),
    );
  ...
  it("marks each item with its group without changing button semantics", async () => {
    ...
    const items = Array.from(nav.querySelectorAll(".kria-navrail__item"));
    expect(items).toHaveLength(7);
    items.forEach((item, index) => {
      expect(item.getAttribute("data-navigation-group")).toBe(SPACE_GROUP[ALL_SPACES[index]]);
...
describe("Dock — Mini icon-only accessible name + reachability (task 7.7)", () => {
  ...
  // Render the rail inside a Mini shell wrapper. The visible
  // `.kria-navrail__label` is CSS-hidden, but the accessible name
